...
const initMap = async () => {
  const background = await loadImage('modern/images/background.svg');
  await Promise.all(deviceCategories.map(async category => {
    if (!map.hasImage(category)) {
      
      const imageData = await loadIcon(category, background, `modern/images/icon/${category}.svg`);
...
const Map = ( { children }) => {
 //console.log(children[0].props.onChange)
